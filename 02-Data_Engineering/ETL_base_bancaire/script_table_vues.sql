...
CREATE TABLE AGENT (
    ...
    nom_agent TEXT,
    ddn_agent DATE,
    civilite_agent CHAR(1) CHECK (civilite_agent IN ('M', 'F'))
);
...
CREATE TABLE CLIENT (
    ...
    nom TEXT,
    prenom TEXT,
    civilite CHAR(1) CHECK (civilite IN ('M', 'F')),
    ddn DATE,
    adresse TEXT,
    cp INT,
    ville TEXT,
    ...
    matricule_agent INT REFERENCES AGENT(matricule_agent)
);
...
CREATE TABLE COMPTE_CLIENT (
    ...
    numcpt BIGINT REFERENCES COMPTE(numcpt),
...
CREATE TABLE PARRAINAGE (
    id_client_parrain INT REFERENCES CLIENT(id_client),
    id_client_filleul INT REFERENCES CLIENT(id_client),
    date_parrainage DATE,
    PRIMARY KEY (id_client_parrain, id_client_filleul)
);

-- Table pour rentrer les données du csv
